...
var start = function () {
    greenTimeout = setTimeout(() => {
        rollOutMenu(".purple", ".firstMenu");
    }, "500");

    tealTimeout = setTimeout(() => {
        rollOutMenu(".teal", ".secondMenu");
    }, "750");

    blueTimeout = setTimeout(() => {
        rollOutMenu(".green", ".thirdMenu");
    }, "1000");

    orangeTimeout = setTimeout(() => {
        rollOutMenu(".orange", ".fourthMenu");
    }, "1250");

    darkpurpleTimeout = setTimeout(() => {
        rollOutMenu(".red", ".fifthMenu");
    }, "1500");

    redTimeout = setTimeout(() => {
        rollOutMenu(".darkpurple", ".sixthMenu");
    }, "1750");
};
...
function collpaseAllOtherMaxed(el) {
    jQuery(".fullWidth.view").each(function (i, v) {
        if (
            jQuery(v).attr("class") !== el.attr("class") &&
            !jQuery(v).hasClass("collapse")
        ) {

            $(v).removeClass("view");
            $(v).find(".option").removeClass("view");
            $(v).closest(".fullWidth").removeClass("collapse");
            $(v).find(".secondary_list").removeClass("expand");
            flipUpSecondary(jQuery(v));
        }
    });
}
...
jQuery(document).on('click', '.option2', function (el) {
    ...
    let clickTarget = getClassNameFromObj(jQuery(el.currentTarget))
    let parentClickTarget = getClassNameFromObj(jQuery(el.currentTarget).parent().parent())
    let distanceFromActiveMenu = jQuery('.view').offset().top;
    console.log(distanceFromActiveMenu)
...
function flipUpSecondary(menu) {
    ...
    console.log(subOptions + " .orange")
    console.log(jQuery(subOptions + " .orange"))
    var tl = anime.timeline({
        duration: 500,
        complete: function () {
            // flipUpSecondary($(closeMenu));
        }
    });
